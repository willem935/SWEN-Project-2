...
public class SmartRobotBehaviour implements IRobotBehaviour {
	...
	public boolean returnToMailRoom(StorageTube tube) {
		...
		if (!tube.isEmpty()) {
			...
			if (priorityCount >= nonPriorityCount) {
				...
				// Check if there is more than 1 priority arrival and the size
				// of the tube is greater than or equal to half
				if (newPriorityArrival > 1
						&& tube.getSize() >= tube.getCapacity() / 2) {
	...
	private class ArrivalComparer implements Comparator<MailItem> {
		 ...
		 * @return -1 if m1 is higher priority, 0 if same and 1 if m2 has higher
		 *priority.
		 */
